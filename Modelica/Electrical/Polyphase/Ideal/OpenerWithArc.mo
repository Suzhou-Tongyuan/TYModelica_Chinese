...
model OpenerWithArc "带电弧的多相开关"
  ...
  Modelica.Electrical.Analog.Ideal.OpenerWithArc openerWithArc[m](
    ...
    each final useHeatPort=useHeatPort) annotation (Placement(
        transformation(extent={{-10,-10},{10,10}}))); // 带电弧的多相开关模型
equation
  ...
  connect(control, openerWithArc.control) 
    annotation (Line(points={{0,120},{0,40},{0,11}}, color={255,0,255})); // 连接控制信号到开关控制端口
...
<p>包含带电弧的m个开关(Modelica.Electrical.Analog.Ideal.OpenerWithArc)。</p>
</html>"));
end OpenerWithArc;