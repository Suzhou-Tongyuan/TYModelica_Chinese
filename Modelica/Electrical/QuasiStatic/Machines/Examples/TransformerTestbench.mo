...
model TransformerTestbench "变压器测试台"
  extends Modelica.Icons.Example;
  parameter Integer m=3 "相数" annotation(Evaluate=true);
  parameter SI.Resistance RL[m]=fill(1/3, m) 
    "负载电阻";
  output SI.Voltage VS=polarVS.len "电网电压";
  output SI.Angle phiS=polarVS.phi "电网相位";
  output SI.Voltage VL=polarVL.len "负载电压";
  output SI.Angle phiL=polarVL.phi "负载相位";
...
您可以选择不同的连接方式以及变化负载（甚至不对称）。<br>
<strong>请注意</strong>正确接地整个电路的主要部分和次要部分。<br>
如果连接不是三角形（D或d），则主要和次要星点可用作连接器。<br>
在某些情况下，即使源的星点或负载的星点接地，也可能需要接地变压器的星点；您可以使用合理高的接地电阻。
</html>"), 
