...
partial model PartialCore 
  ...
  parameter Real n12(start=1) "变比 1:2";
  parameter Real n13(start=1) "变比 1:3";
...
equation
  ...
  //定义 p1.reference.gamme = p2.reference.gamma = p3.reference.gamma
...
具有3个绕组的变压器铁心的部分模型；必须定义饱和函数磁通与磁化电流之间的关系。
</html>"));
end PartialCore;