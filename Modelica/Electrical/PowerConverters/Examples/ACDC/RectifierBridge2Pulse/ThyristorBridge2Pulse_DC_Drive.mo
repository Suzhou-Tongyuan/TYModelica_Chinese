...
model ThyristorBridge2Pulse_DC_Drive 
  ...
  final parameter SI.Torque tauNominal=dcpmData.ViNominal 
      *dcpmData.IaNominal/dcpmData.wNominal "额定扭矩";
  output SI.AngularVelocity w(displayUnit="rpm") = dcpm.wMechanical "驱动的角速度";
  output SI.Torque tau=dcpm.tauShaft "驱动的轴扭矩";
...
<p>
在这个示例中，一个PM励磁直流机以额定扭矩和额定转速启动。在5秒后，负载扭矩在额外的10秒内减少到零。在15秒时，机器处于无载荷运行状态。
</p>

<p>
绘制扭矩 <code>tau</code>，电流 <code>currentSensor.i</code> 和平均电流 <code>meanCurrent.y</code>。还绘制速度 <code>w</code>，电压 <code>voltageSensor.v</code> 和平均电压 <code>meanVoltage.y</code>。</p>
</html>"));
