...
model PMOS "带热端口的简单PMOS栅极管"
  ...
  parameter SI.Length W = 20.0e-6 "宽度";
  parameter SI.Length L = 6.0e-6 "长度";
  parameter SI.Transconductance Beta = 0.0105e-3 "转导系数参数";
  parameter SI.Voltage Vt = -1.0 "零偏置阈值电压";
  parameter Real K2 = 0.41 "体阈值参数";
  parameter Real K5 = 0.839 "截断区域的减小";
  parameter SI.Length dW = -2.5e-6 "通道窄化";
  parameter SI.Length dL = -2.1e-6 "通道缩短";
  ...
  parameter Real kvt = -2.9e-3 "Vt的适应参数" annotation(Dialog(enable = useTemperatureDependency));
  parameter Real kk2 = 6.2e-4 "K2的适应参数" annotation(Dialog(enable = useTemperatureDependency));
...
<p>为热电模拟，模型添加了一个热端口。加热端口定义在Modelica.Thermal库中。
</p>

<p>该模型没有考虑电容。为避免发生数值上的问题，该模型已包含了高的源阻抗RDS。</p>
