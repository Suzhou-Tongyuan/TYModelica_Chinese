...
model ResonanceCircuits 
  ...
  final parameter SI.Frequency fRes = 1 / (2 * pi * sqrt(L * C)) "源频率";
...
<p>
这个示例演示了如何通过物理连接器和输入/输出信号之间的适配器来连接并联共振电路和串联共振电路的组件，而不是直接连接。
考虑到所需的导数，这些组件可以导出为输入/输出模块。(例如：<a href=\"https://fmi-standard.org\">Functional Mock-up Unit</a>)。
电路中连接这些输入/输出模块长应该会产生与\"直接物理连接的电路形式\"相同的结果。
</p>
<p>
请记住，用户应将物理组件分离并通过适配器信号连接时，应该需要放置适当的<a href=\"modelica://Modelica.Electrical.Analog.Basic.Ground\">接地组件</a>。
</p>
</html>"), 
