...
model CauerLowPassSC "使用运算放大器和开关电容器实现的Cauer低通滤波器"
...
<p>CauerLowPassSC是一个五阶低通滤波器。它采用带运算放大器的开关电容网络实现。电压源是滤波器的输入电压(阶跃)，OP5.out.v 是滤波器输出电压。 该滤波器已计算脉冲响应。</p>
<p>该模型与CauerLowPassAnalog示例类似，但是是反向的(两者的输出电压变化趋势相反)。为了获得与CauerLowPassAnalog示例相同的响应，使用负电压(步进)作为输入。</p>
<p>该模型与CauerLowPassOPV示例类似，但是电阻由开关电容实现(详见<a href=\"modelica://Modelica.Electrical.Analog.Examples.Utilities.SwitchedCapacitor\">SwitchedCapacitor</a>)。滤波器电阻值有两种,一种值为<code>R=1</code>，另一个值为<code>R=-1</code>。</p>
<p>CauerLowPassSC示例的仿真结束时间为60秒。用户可在“仿真--仿真浏览器”中勾选相应的“仿真结果变量”查看示例的输入电压和输出电压。用户可以将此结果与CauerLowPassAnalog的运行结果进行比较。</p>
<p>由于电容的充电现象，该滤波器示例的仿真性能会不如CauerLowPassOPV示例。</p>
</html>"));
end CauerLowPassSC;