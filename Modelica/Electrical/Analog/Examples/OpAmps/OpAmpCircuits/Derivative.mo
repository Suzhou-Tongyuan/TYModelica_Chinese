...
model Derivative "低通滤波器运算放大器电路"
  extends PartialOpAmp;
  import Modelica.Constants.pi;
  parameter Real k(final min = 0) = 1 "期望的增益";
  parameter SI.Resistance R1 = 1000 "运算放大器负输入端的电阻";
  parameter SI.Resistance R2 = k * R1 "计算得到的电阻，以达到增益k";
  parameter SI.Time T "时间常数";
  parameter SI.Capacitance C = T / R1 "计算得到的电容，以达到时间常数T";
  SI.Voltage v(start = 0) = c.v "电容器电压 = 状态";
