...
partial model FourPin "具有两对(每对两个)电气引脚的组件"
  SI.Voltage v1 "1端口电压降 (= p1.v - n1.v)";
  SI.Voltage v2 "2端口电压降 (= p2.v - n2.v)";
  SI.Current i1 "从1端口正极流向负极端口的电流";
  SI.Current i2 "从2端口正极流向负极端口的电流";
  PositivePin p1 "1端口正极" annotation(Placement(
    transformation(extent = {{-110, 90}, {-90, 110}}), iconTransformation(extent = {{-110, 90}, {-90, 110}})));
  NegativePin n1 "1端口负极" annotation(Placement(
    transformation(extent = {{-90, -110}, {-110, -90}}), iconTransformation(extent = {{-90, -110}, {-110, -90}})));
  PositivePin p2 "2端口正极" annotation(Placement(
    transformation(extent = {{110, 90}, {90, 110}}), iconTransformation(extent = {{110, 90}, {90, 110}})));
  NegativePin n2 "2端口负极" annotation(Placement(
    transformation(extent = {{90, -110}, {110, -90}}), iconTransformation(extent = {{90, -110}, {110, -90}})));
equation
  v1 = p1.v - n1.v;
  v2 = p2.v - n2.v;
...
<li><em> 1998   </em>
    ...
    info = "<html>
<p>FourPin是一个部分模型，它由两对(每对两个引脚)电气引脚组成。
</p>
