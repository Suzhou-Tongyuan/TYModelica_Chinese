...
model CosineCurrentVariableFrequencyAndAmplitude 
  ...
  parameter Boolean useConstantAmplitude=false "使电流源保持恒定幅度";
  parameter SI.Current constantAmplitude=1 "恒定振幅" 
  ...
  parameter SI.Current offset=0 "正弦波的偏置";
...
protected
  ...
  Blocks.Sources.Constant f_constant(final k=constantFrequency) if useConstantFrequency 
    annotation (Placement(transformation(extent={{-20,70},{-40,90}})));
  Blocks.Interfaces.RealInput I_internal(unit="A") "幅值" annotation (Placement(
        transformation(
        extent={{-2,-2},{2,2}}, 
        rotation=270, 
        origin={60,80})));
...
<p>这个电流源提供了一个正弦电流，其频率<code>f</code>和幅度<code>I</code>都是可变的，即正弦波的相位角是从2πf积分得到的。
</p>

<p>请注意，初始相位角<code>phi</code>的值定义了初始相位偏移。参数<code>startTime</code>被省略，因为通过将输入电流<code>I</code>设置为零，可以保持电压始终等于offset，从而实现电压的恒定不变，即从零点开始。
</p>
</html>"));
