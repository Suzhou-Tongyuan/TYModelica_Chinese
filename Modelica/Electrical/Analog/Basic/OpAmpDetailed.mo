...
model OpAmpDetailed "运算放大器的详细模型"
  ...
  parameter SI.Voltage Vos = 1.0e-3 "输入偏置电压";
  parameter SI.Current Ib = 80.0e-9 "输入偏置电流";
  parameter SI.Current Ios = 20.0e-9 "输入偏置电流差";
  parameter SI.Voltage vcp = 0.0 "由p_supply限值的修正值";
  parameter SI.Voltage vcm = 0.0 "由msupply限值的修正值";
  parameter Real Avd0 = 106.0 "差分放大器[dB]";
  parameter Real CMRR = 90.0 "共模抑制[dB]";
  parameter SI.Frequency fp1 = 5.0 "主极点";
  parameter SI.Frequency fp2 = 2.0e6 "极点频率";
  parameter SI.Frequency fp3 = 20.0e6 "极点频率";
  parameter SI.Frequency fp4 = 100.0e6 "极点频率";
  parameter SI.Frequency fz = 5.0e6 "零频率";
  ...
  //  Ib = 0.5*(I1 + I2);
  //  Ios = I1 - I2;
  final parameter SI.Current I1 = Ib + Ios / 2.0 "内部源I1的电流";
  final parameter SI.Current I2 = Ib - Ios / 2.0 "内部源I2的电流";
...
<p>内部限制函数，专为OpAmpDetailed设计，不用于外部使用。</p>
</html>"  ));
  end FCNiout_limit;

  function FCNq_sum_limit "内部限制函数"
    extends Modelica.Icons.Function;
  ...
  algorithm
    ...
    return;
    annotation(Documentation(info = "<html>
<p>内部限制函数，专门设计用于运算放大器OpAmpDetailed，不适用于外部使用。</p>
</html>"  ));
  end FCNq_sum_limit;
...
<p>根据[<a href=\"modelica://Modelica.Electrical.Analog.UsersGuide.References\">Conelly1992</a>]，Joachim Haase(Fraunhofer Institute for Integrated Circuits, Design Automation Division)于2001年将运算放大器模型转换为VHDL-AMS。现在，这些模型中的一个，即“amp(macro)”模型已被转换为Modelica。</p>
</html>", revisions = "<html>
<dl>
