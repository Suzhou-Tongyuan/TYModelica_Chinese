...
model Potentiometer "可变电阻"
  parameter SI.Resistance R(start=1) 
    "在温度T_ref时的电阻";
  parameter SI.Temperature T_ref=293.15 "参考温度";
  parameter SI.LinearTemperatureCoefficient alpha=0 
    "电阻温度系数(R_actual = R*(1 + alpha*(T_heatPort - T_ref))";
  extends Modelica.Electrical.Analog.Interfaces.ConditionalHeatPort(T=T_ref);
  parameter Boolean useRinput=false "使用输入信号控制位置0<r<1(否则为常数)" 
    annotation (
    Evaluate=true, 
    HideResult=true, 
    Dialog(group="电位器"));
  parameter Real rConstant(
    final min=0, 
    final max=1) = 0.5 "在n(r=0)和p(r=1)之间的接触点" 
    annotation (Dialog(group="电位器", enable=not useRinput));
  SI.Resistance Rp 
    "端点pin_p与接触点之间的实际电阻";
  SI.Resistance Rn 
    "接触点与端点pin_n之间的实际电阻";
...
equation
  ...
  annotation (
    ...
    Documentation(info="<html>
                       <p>这个模型描述了一个电位器，其中滑动接触点位于引脚_n(电阻r=0)和引脚_p(电阻r=1)之间，取决于参数rConstant或信号输入r。</p>
                       <p>总电阻R是温度依赖性的。</p>
                       </html>"));
end Potentiometer;