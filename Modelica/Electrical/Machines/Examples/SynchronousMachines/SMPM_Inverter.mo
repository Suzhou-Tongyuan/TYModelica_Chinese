...
model SMPM_Inverter 
  ...
  parameter Utilities.ParameterRecords.SM_PermanentMagnetData smpmData "同步电机数据" 
...
equation
  ...
  annotation (experiment(StopTime=1.5, Interval=1E-4, Tolerance=1e-06), Documentation(
        info="<html>
<p>通过使用 VfController 和三相 SignalVoltage 模拟理想的频率逆变器。
通过斜坡提高频率，导致永磁同步电机启动，并加速惯性。在时间 tStep，加载步进被应用。</p>

<p>示例的仿真时长为1.5秒，用户可以在特定界面通过勾选绘制以下结果变量的图像(相对于时间)：</p>

<ul>
<li>currentQuasiRMSSensor.I：定子电流 RMS</li>
<li>smpm.wMechanical：电机速度</li>
<li>smpm.tauElectrical：电机转矩</li>
<li>rotorDisplacementAngle.rotorDisplacementAngle：转子位移角度</li>
</ul>

<p>使用默认机器参数。</p>

<p>在实践中，几乎不可能驱动永磁同步电机而没有电流控制器。</p>
</html>"));
end SMPM_Inverter;