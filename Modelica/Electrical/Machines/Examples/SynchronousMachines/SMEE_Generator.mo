...
model SMEE_Generator 
  ...
  parameter SI.AngularVelocity wActual(displayUnit="rev/min")= 
       1499*2*Modelica.Constants.pi/60 "实际转速";
...
<p>一个电激励同步发电机连接到电网并以恒定速度驱动。
由于速度略小于与主频对应的同步速度，转子角度被非常缓慢地增加。
这允许查看几个依赖于转子角度的特性。</p>

<p>该示例的仿真时长为30秒，用户可以在特定界面通过勾选绘制以下变量的图像(相对于rotorDisplacementAngle.rotorDisplacementAngle)：</p>
