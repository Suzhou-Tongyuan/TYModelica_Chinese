...
model SMPM_ResistiveBraking 
  ...
  parameter SI.Inertia JLoad=4*smpmData.Jr "负载的转动惯量";
  parameter SI.AngularVelocity w0(displayUnit="rev/min")= 
    2*pi*smpmData.fsNominal/smpmData.p "初始速度";
  parameter Real k[3]={1,3,5} "相对于 Rs 的制动电阻级数";
  parameter SI.Current idq_sr[2](each fixed=false) 
    "初始定子电流空间矢量";
  SI.Torque tauElectrical=smpm.tauElectrical "电磁转矩";
  SI.Torque tauShaft=smpm.tauShaft "轴转矩";
  SI.AngularVelocity wMechanical(displayUnit="rev/min")= 
    smpm.wMechanical "轴速度";
  parameter
...
equation
  ...
  annotation (experiment(StopTime=1.0, Interval=0.001, Tolerance=1e-06),              Documentation(
        info="<html>
<p>
永磁同步电机的永磁体感应的电压缩短在定子绕组的电感和电阻以及(可选的)外部制动电阻上。这些电压驱动的电流引起制动转矩。
</p>
<p>
外部制动电阻采用三个级别实现，它们在制动过程的不同阶段被缩短。注意，第一个(最小的)级别不被缩短，这确保了最小阻尼，以避免轴速度围绕零点振荡。
总制动电阻(所有级别的总和)必须根据制动开始时的轴速度进行调整。
</p>
<p>
用户可以在特定界面绘制tauElectrical和tauShaft相对于wMechanical的图像。
</p>
<p>使用默认的机器参数。</p>
</html>"));
