...
model DcdcInverter "直流-直流逆变器"
  parameter Boolean useIdealInverter=true "使用理想平均逆变器，否则使用开关逆变器";
  parameter SI.Frequency fS "开关频率";
  parameter SI.Time Td=0.5/fS "死区时间";
  parameter SI.Time Tmf=2/fS "测量滤波器时间常数";
  parameter SI.Voltage VMax "最大电压";
  parameter SI.Time Ti=1e-6 "积分功率控制器的时间常数" 
  ...
  parameter SI.Conductance GoffD=1e-05 
    "二极管截止电导" 
    annotation (Dialog(group="Switching", enable=not useIdealInverter));
  parameter SI.Voltage VkneeD=0 "二极管阈值电压" 
    annotation (Dialog(group="Switching", enable=not useIdealInverter));
  output SI.Power pDC=vDC*iDC "直流功率（来自电池）";
  output SI.Power pMot=vMot*iMot "电机功率";
...
<p>这是一个直流-直流逆变器模型。直流-直流逆变器的详细级别可以选择理想平均或开关。</p>
<p>参考电压限制为实际电池电压。</p>
<p>测量电池电压和电机电流。</p>
