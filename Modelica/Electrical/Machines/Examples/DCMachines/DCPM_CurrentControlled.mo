...
model DCPM_CurrentControlled 
  ...
  parameter SI.Time Ts=1e-3 "逆变器的死时间";
  parameter SI.Resistance k=Ra*Ta/(2*Ts) "电流控制器比例增益";
...
equation
  ...
  connect(dcpm.pin_an, ground.p) 
    annotation (Line(points={{4,-10},{4,-1.77636e-15},{-8,-1.77636e-15}}, color={0,0,255}));
  ...
  connect(currentSensor.i, PID.u_m) 
    annotation (Line(points={{31,10},{100,10},{100,-82},{-40,-82},{-40,48}}, color={0,0,127}));
  connect(speedSensor.w, PID.u_ff) 
    annotation (Line(points={{30,-49},{30,-70},{-34,-70},{-34,48}}, color={0,0,127}));
  annotation (experiment(StopTime=2.0, Interval=1E-4, Tolerance=1E-6), Documentation(
        info="<html>

<p>电流控制器根据绝对最优进行参数化。
在0.1秒时，应用高度为额定电枢电流的参考电流阶跃，导致直流机启动，并加速惯性负载。
负载由二次速度相关的负载转矩加载。</p>
示例的仿真时间为2秒，用户可以在特定界面绘制以下结果变量的图像(相对于时间):
<ul>
<li>dcpm.ia：电枢电流</li>
<li>dcpm.wMechanical：电机转速</li>
<li>dcpm.tauElectrical：电机转矩</li>
</ul>
使用模型<em>DC_PermanentMagnet</em>的默认机械参数。
</html>"));
