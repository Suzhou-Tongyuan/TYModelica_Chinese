...
model IMC_Initialize 
  ...
  parameter SI.AngularVelocity wSync=2*pi*fNominal/aimc.p "同步速度";
  parameter SI.Time tStart=0.5 "开始时间";
  parameter SI.Torque TLoad=161.4 "额定负载转矩";
...
equation
  ...
  annotation (experiment(StopTime=1.5, Interval=1E-4, Tolerance=1e-06), Documentation(info="<html><p>
鼠笼型感应电机在空载时稳态初始化； 在时间 tStart 时施加负载转矩步进。
</p>
<p>
该示例的仿真时长为1.5秒，用户可以在特定窗口通过勾选绘制以下(与时间相关的)结果变量的图像：
</p>
<li>
currentQuasiRMSSensor.I：定子电流 RMS 值</li>
<li>
aimc.wMechanical：电机速度</li>
<li>
aimc.tauElectrical：电机转矩</li>
<p>
使用默认的电机参数。
</p>
</html>"));
