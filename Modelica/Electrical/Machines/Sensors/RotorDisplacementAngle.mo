...
model RotorDisplacementAngle "转子滞后角"
  parameter Integer m=3 "相数" annotation(Evaluate=true);
  parameter Integer p(min=1) "极对数";
  parameter Boolean positiveRange=false "如果为真，则仅使用正输出范围";
  parameter Real threshold(final min=0)=0 "低于阈值时，电压被视为零";
  parameter Boolean useSupport=false "使用支撑或固定壳体" 
  ...
  Modelica.Mechanics.Rotational.Interfaces.Flange_a support if useSupport 
    "反扭矩作用处的支撑" annotation (Placement(
...
通过测量定子相电压，将其转换为定子固定坐标系中的相应空间矢量，
将空间矢量旋转到转子固定坐标系并计算此空间矢量的角度来计算转子滞后角。
<p>
传感器的外壳可以隐式固定(useSupport=false)。
如果机器的定子也隐式固定(useSupport=false)，则法兰处的角度等于定子相对于转子的角度。
否则，传感器的支撑必须连接到机器的支撑。
</p>
</html>"));
