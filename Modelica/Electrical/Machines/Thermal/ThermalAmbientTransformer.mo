...
model ThermalAmbientTransformer "变压器的热环境"
  ...
  Modelica.Blocks.Interfaces.RealInput TPrimary(unit="K") if useTemperatureInputs 
    "初级绕组的温度" annotation (Placement(transformation(
        extent={{-20,-20},{20,20}}, 
        rotation=90, 
...
用于变压器的热环境，可以通过常数或信号连接器指定绕组温度。此外，所有损耗(热流)已经被记录。
</html>"));
