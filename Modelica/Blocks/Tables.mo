  ...
  equation
    if tableOnFile then
      assert(tableName <> "NoName", 
        "tableOnFile = true但未给出表格名称");
    else
      assert(size(table, 1) > 0 and size(table, 2) > 0, 
        "tableOnFile = false但参数表为空矩阵");
    end if;
...
在<strong>表格</strong>的<strong>一维</strong>上进行<strong>单变量</strong>的<strong>常数</strong>、<strong>线性</strong>或<strong>三次Hermite样条插值</strong>。
通过参数<strong>columns</strong>，可以定义对表格中的多少列进行插值。
例如，如果columns = {2, 4}，则假定有2个输出信号，第1个输出信号通过表格矩阵的第2列进行插值，第2个输出信号通过第4列进行插值。
</p>
<p>
网格点和函数值存储在矩阵\"table[i, j]\"中，其中第1列\"table[:,1]\"包含网格点，其他列包含待插值的数据。示例：
</p>
<blockquote><pre>
table = [0,  0;
         1,  1;
...
<ul>
<li>插值区间通过二分查找找到，其中上次调用使用的区间作为起始区间。</li>
<li>通过参数<strong>smoothness</strong>可以定义数据的插值方式：
<blockquote><pre>
...
<li>提供一阶和二阶<strong>导数</strong>，但以下两种平滑选项除外。
<ol>
<li>常数段插值不提供导数。</li>
<li>线性插值不提供二阶导数。存在设计不一致性，使用线性插值和重复的采样点可以模拟由常数段组成的信号。与常数段插值不同，线性插值提供的第一导数为零。</li>
</ol></li>
<li><strong>超出</strong>表格范围的值，根据参数<strong>extrapolation</strong>的设置进行外推计算：
<blockquote><pre>
...
<li>如果表格只有<strong>一行</strong>，则返回该表格值，与输入信号的值无关。</li>
<li>网格点的值(第1列)必须严格递增。</li>
</ul>
<p>
表格矩阵可以通过以下方式定义：
</p>
<ol>
<li>显式地将<strong>矩阵</strong>\"table\"作为<strong>参数</strong>提供，其他参数的值如下：
<blockquote><pre>
tableName is \"NoName\" or has only blanks,
fileName  is \"NoName\" or has only blanks.
</pre></blockquote></li>
<li>从<strong>文件</strong>\"fileName\"<strong>读取</strong>，其中矩阵存储为\"tableName\"。
    可以是文本格式或MATLAB MAT文件格式(文本格式如下所述)。
    MAT文件格式有四个不同版本：v4、v6、v7 和 v7.3。
    该库至少支持v4、v6 和 v7，而v7.3是不支持的。
    当模型中需要使用三个表格tab1、tab2、tab3时，最方便的方式是通过FreeMat或MATLAB®的命令
<blockquote><pre>
save tables.mat tab1 tab2 tab3
</pre></blockquote>
    生成MAT文件，或者通过Scilab的命令
<blockquote><pre>
savematfile tables.mat tab1 tab2 tab3
</pre></blockquote>
    生成。<br>
    请注意，可以通过使用辅助函数
    <a href=\"modelica://Modelica.Utilities.Files.loadResource\">loadResource</a>将fileName定义为URI。</li>
<li>静态存储在文件\"usertab.c\"中的函数\"usertab\"中。
    矩阵通过\"tableName\"进行标识。
    参数fileName = \"NoName\"或仅包含空格。始终优先使用按行存储，因为否则矩阵将被重新分配并转置。
    请查看<a href=\"modelica://Modelica.Blocks.Tables\">Tables</a>包的文件获取更多细节。</li>
</ol>
<p>
当定义常量\"NO_FILE_SYSTEM\"时，所有与文件I/O相关的源代码部分将被C预处理器移除，从而不再进行文件访问。
</p>
<p>
如果从文本文件中读取表格，则文件需要具有以下结构(\"-----\"不是文件内容的一部分)：
</p>
<blockquote><pre>
-----------------------------------------------------
#1
...
<p>
如果从csv文件中读取表格，使用文本编辑器打开的文件具有以下结构(\"-----\"不是文件内容的一部分)：
</p>
<blockquote><pre>
-----------------------------------------------------
#1
...
<p>
请注意，文件中的前两个字符需要是\"#1\"(行注释，用于定义文件格式的版本号)。
之后，需要声明相应的矩阵，包含类型(如\"double\"或\"float\")、名称和实际维度。
最后，在文件的连续行中，必须给出矩阵的元素。
这些元素必须按行优先顺序作为数字序列提供(因此，一个矩阵的行可以跨越文件中的多行，并且不需要从行的开头开始)。
数字必须按照C语法给出(例如2.3，-2，+2.e4)。
数字分隔符可以是空格、制表符(\\t)、逗号(,)或分号(;)。
多个矩阵可以一个接一个地定义。
行注释以井号(#)开始，并可以出现在任何地方。
文本文件应为ASCII或UTF-8编码，其中UTF-8编码的字符串仅允许出现在行注释中，且文本文件开头的可选UTF-8 BOM会被忽略。
文件中不允许其他字符，如尾随非注释部分。
</p>
<p>
...
"          , level = AssertionLevel.warning);
    ...
    annotation(
      Documentation(info = "<html><p>
在<strong>表格</strong>的<strong>一维</strong>上进行<strong>单变量常数</strong>、<strong>线性</strong>或<strong>三次Hermite样条插值</strong>。
通过参数<strong>columns</strong>，可以定义对表格中的多少列进行插值。
例如，如果columns = {2, 4}，则假定有2个输入信号和2个输出信号，第1个输出信号通过第1个输入信号在表格矩阵的第2列进行插值，第2个输出信号通过第2个输入信号在表格矩阵的第4列进行插值。
</p>
<p>
网格点和函数值存储在矩阵\"table[i, j]\"中，其中第1列\"table[:,1]\"包含网格点，其他列包含待插值的数据。示例：
</p>
...
"      , level = AssertionLevel.warning);
    ...
    annotation(
      Documentation(info="<html><p>
对<strong>二维表格</strong>进行<strong>双变量常数插值</strong>、<strong>双线性插值</strong>、<strong>双变量Akima插值</strong>。
网格点和函数值存储在矩阵\"table[i,j]\"中，其中：
</p>
<ul><li>
第一列\"table[2:,1]\"包含u1网格点，</li>
<li>
第一行\"table[1,2:]\"包含u2网格点，</li>
<li>
其他行和列包含待插值的数据。</li>
</ul><p>
例如:
...
<li>如果表格只有<strong>一个元素</strong>，则返回该表格值，与输入信号的值无关。</li>
<li>网格点的值(第1列和第1行)必须严格递增。</li>
</ul>
<p>
表格矩阵可以通过以下方式定义：
</p>
...
文本文件应为ASCII或UTF-8编码，其中UTF-8编码的字符串仅允许出现在行注释中，且文本文件开头的可选UTF-8 BOM会被忽略。
文件中不允许其他字符，如尾随非注释部分。
矩阵元素的解释方式与矩阵作为参数给出时完全相同。
例如，第1列\"table2D_1[2:,1]\"包含u[1]网格点，第1行\"table2D_1[1,2:]\"包含u[2]网格点。
</p>
<p>
MATLAB是MathWorks公司的注册商标。
</p>
  ...
  package NTables "用于在N维表格中插值的块库"
    ...
    model LookupTable1D "一维线性插值表"
      ...
      parameter String filePath = "none" "数据存储的文件路径" 
        ...
        loadSelector(filter = "逗号分隔符文件(*.csv)", caption = "打开存在表格的文件")));
    ...
    protected
      parameter Integer bp_sizes[:] = {size(breakPoints1, 1)} "表格中需要插值的数据列";
      parameter Tables.Types.External1DTable externalTable = Tables.Types.External1DTable(filePath, tableData_col, bp_cols, breakPoints1, bp_sizes, tableData);
...
<p>
<span style=\"color: rgb(51, 51, 51);\">该模块通过查找或插入您使用模块参数定义的值表，将输入映射到输出值。
支持的插值方法有：齐次(常数)、线性(线性点斜式)、最近邻、Akima样条、Fritsch-Butland样条、Steffen样条插值方法，以及齐次(常数)、线性(线性点斜式)外推方法。</span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51);\">在下表中，第一列标识breakPoints1，第二列标识表格数据。</span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51);\">例如，当breakPoints1为{-1, 0, 1.2}，对应的值为{1, 2, 3}时，您可以直接将tableData设置为{1, 2, 3}。</span>
</p>
<p>
...
<br>
    ...
    protected
      parameter Integer bp_sizes[:] = {size(breakPoints1, 1), size(breakPoints2, 1)} "表格中需要插值的数据列";
      parameter Real tableDataVec[size(breakPoints1, 1) * size(breakPoints2, 1)] = Utilities.arr2D2vec(tableData);
      parameter Tables.Types.External2DTable externalTable = Tables.Types.External2DTable(filePath, tableData_col, bp_cols, breakPoints1, breakPoints2, bp_sizes, tableDataVec);
...
<p>
<span style=\"color: rgb(51, 51, 51);\">该模块通过查找或插入您使用模块参数定义的值表，将输入映射到输出值。支持的插值方法包括均匀(常数)、线性(线性点斜率)、最近邻、Akima样条、Fritsch-Butland样条、Steffen样条插值方法，以及均匀(常数)、线性(线性点斜率)外推方法。</span>
</p>
<p>
...
<img src=\"data:image/png;base64,\" alt=\"image.png\" data-href=\"\" style=\"\"/><br>
    ...
    protected
      parameter Integer bp_sizes[:] = {size(breakPoints1, 1), size(breakPoints2, 1), size(breakPoints3, 1)} "表格中需要插值的数据列";
      parameter Real tableDataVec[size(breakPoints1, 1) * size(breakPoints2, 1) * size(breakPoints3, 1)] = Utilities.arr3D2vec(tableData);
      parameter Tables.Types.External3DTable externalTable = Tables.Types.External3DTable(filePath, tableData_col, bp_cols, breakPoints1, breakPoints2, breakPoints3, bp_sizes, tableDataVec);
    ...
    equation
      ...
      annotation(Documentation(info="<html><p>
<span style=\"color: rgb(51, 51, 51);\">执行一维插值表查找，其中n = 1。该表是函数中N个变量的采样表示。断点集将输入值与表中的位置关联起来。</span><br><span style=\"color: rgb(51, 51, 51);\">一维查找表、二维查找表和n维查找表计算函数的 n 变量采样表示：</span>
</p>
<p>
<span data-w-e-type=\"formula\" data-w-e-is-void data-w-e-is-inline data-value=\"y=F(x_1,x_2,x_3,...,x_N)\"></span><br><span style=\"color: rgb(51, 51, 51);\">其中函数 F 是一个经验函数。</span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51);\">该模块通过查找或插入您使用模块参数定义的值表，将输入映射到输出值。支持的插值方法包括：齐次(常数)、线性(线性点斜式)、最近邻、Akima样条、Fritsch-Butland样条、Steffen样条插值方法，以及齐次(常数)、线性(线性点斜式)外推方法。</span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51);\">3D查找表是通过叠加和扩展2D查找表得到的。3D查找表的每一页都是一个2D查找表。</span>
</p>
<p>
<span style=\"color: rgb(51, 51, 51);\">例如，当breakPoints1为{-1, 0, 1.2}，breakPoints2为{-2.3, -1.07, 0.05}，对应的值为{1, 2, 3, 4, 5, 6, 7, 8, 9}时，可以设置表格数据为{{1, 4, 7}, {2, 5, 8}, {3, 6, 9}}。</span>
</p>
<p>
...
<p>
<span style=\"color: rgb(51, 51, 51);\">在以下模块中，breakPoints 1由表格每一页的第一列确定，breakPoints 2由表格每一页的第一行确定，breakPoints 3由表格的各页确定，tableData由其他部分确定。</span>
</p>
<p>
<img src=\"data:image/png;base64,\" alt=\"image.png\" data-href=\"\" style=\"\"/>
  ...
  package Internal "用户不应直接使用的表格功能的内部外部对象定义"
    ...
    function readTimeTableData "从文本或MATLAB MAT文件读取表格数据"
...
特别是在<strong>no file system</strong>的实时平台目标(如HIL模拟器)上使用时，
可以使用函数\"usertab\"将表静态存储在一个常规命名为\"usertab.c\"的文件中。
这比将表格作为Modelica参数数组提供更有效率。
</p>
<p>
具体做法是以C代码的形式提供特定结构的表格，并将C代码与仿真模型的其他部分一起编译成可在目标平台上执行的二进制文件。
MSL安装目录下的\"Resources/Data/Tables/\"子目录包含<a href=\"modelica://Modelica/Resources/Data/Tables/usertab.c\" target=\"\">\"usertab.c\"</a>和<a href=\"modelica://Modelica/Resources/Data/Tables/usertab.h\" target=\"\">\"usertab.h\"</a>文件，
可用作开发模板。
\"usertab.c\"一般不加修改即可使用，而\"usertab.h\"则需要根据自己的需要进行调整。
</p>
<p>
为了正常工作，编译器必须调入\"usertab.c\"文件。
不同的Modelica工具可能提供不同的机制。
请查阅您的Modelica工具的相关文档/支持。
</p>
<p>
一种可行(但略显临时)的方法是通过使用Modelica外部函数接口的\"dummy\"函数来调入所需的\"usertab.c\"文件。
下面给出了一个例子。
</p>
<blockquote><pre>
