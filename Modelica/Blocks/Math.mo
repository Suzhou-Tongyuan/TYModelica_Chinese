...
<br>
  ...
  block Add "输出两个输入的总和"
    ...
    Modelica.Blocks.Interfaces.RealInput u1 if dataType == Modelica.Blocks.Types.BasicDataTypes.Real "实型输入信号1接口" 
      annotation(Placement(transformation(extent = {{-140, 40}, {-100, 80}})));
    Modelica.Blocks.Interfaces.RealInput u2 if dataType == Modelica.Blocks.Types.BasicDataTypes.Real "实型输入信号2接口" 
      annotation(Placement(transformation(extent = {{-140, -80}, {-100, -40}})));
    Modelica.Blocks.Interfaces.IntegerInput u1_Integer if dataType == Modelica.Blocks.Types.BasicDataTypes.Integer "整型输入信号1接口" 
      annotation(Placement(transformation(extent = {{-140, 40}, {-100, 80}})));
    Modelica.Blocks.Interfaces.IntegerInput u2_Integer if dataType == Modelica.Blocks.Types.BasicDataTypes.Integer "整型输入信号2接口" 
      annotation(Placement(transformation(extent = {{-140, -80}, {-100, -40}})));
    Modelica.Blocks.Interfaces.BooleanInput u1_Boolean if dataType == Modelica.Blocks.Types.BasicDataTypes.Boolean "布尔型输入信号1接口" 
      annotation(Placement(transformation(extent = {{-140, 40}, {-100, 80}})));
    Modelica.Blocks.Interfaces.BooleanInput u2_Boolean if dataType == Modelica.Blocks.Types.BasicDataTypes.Boolean "布尔型输入信号2接口" 
      annotation(Placement(transformation(extent = {{-140, -80}, {-100, -40}})));
    Modelica.Blocks.Interfaces.RealOutput y "实型输出信号接口" 
...
<br>
  ...
  block Add3 "输出三个输入的总和"
    ...
    Interfaces.RealInput u1 if dataType == Modelica.Blocks.Types.BasicDataTypes.Real "实型输入信号1接口" 
      annotation(Placement(transformation(extent = {{-140, 60}, {-100, 100}})));
    Interfaces.RealInput u2 if dataType == Modelica.Blocks.Types.BasicDataTypes.Real "实型输入信号2接口" 
      annotation(Placement(transformation(extent = {{-140, -20}, {-100, 20}})));
    Interfaces.RealInput u3 if dataType == Modelica.Blocks.Types.BasicDataTypes.Real "实型输入信号3接口" 
      annotation(Placement(transformation(extent = {{-140, -100}, {-100, -60}})));
    Interfaces.IntegerInput u1_Integer if dataType == Modelica.Blocks.Types.BasicDataTypes.Integer "整型输入信号1接口" 
      annotation(Placement(transformation(extent = {{-140, 60}, {-100, 100}})));
    Interfaces.IntegerInput u2_Integer if dataType == Modelica.Blocks.Types.BasicDataTypes.Integer "整型输入信号2接口" 
      annotation(Placement(transformation(extent = {{-140, -20}, {-100, 20}})));
    Interfaces.IntegerInput u3_Integer if dataType == Modelica.Blocks.Types.BasicDataTypes.Integer "整型输入信号3接口" 
      annotation(Placement(transformation(extent = {{-140, -100}, {-100, -60}})));
    Interfaces.BooleanInput u1_Boolean if dataType == Modelica.Blocks.Types.BasicDataTypes.Boolean "布尔型输入信号1接口" 
      annotation(Placement(transformation(extent = {{-140, 60}, {-100, 100}})));
    Interfaces.BooleanInput u2_Boolean if dataType == Modelica.Blocks.Types.BasicDataTypes.Boolean "布尔型输入信号2接口" 
      annotation(Placement(transformation(extent = {{-140, -20}, {-100, 20}})));
    Interfaces.BooleanInput u3_Boolean if dataType == Modelica.Blocks.Types.BasicDataTypes.Boolean "布尔型输入信号3接口" 
      annotation(Placement(transformation(extent = {{-140, -100}, {-100, -60}})));
    Interfaces.RealOutput y "实型输出信号接口" 
      annotation(Placement(transformation(extent = {{100, -10}, {120, 10}})));
  ...
  equation
    ...
    annotation(
      Documentation(info = "<html><p>
该模块将输出<strong>y</strong>计算为三个输入信号 <strong>u1</strong>、<strong>u2</strong>和<strong>u3</strong>之<em>和</em>：
</p>
...
<img src=\"modelica://Modelica/Resources/Images/Math/tan.png\"
  ...
  equation
    ...
    annotation(
      ...
      Documentation(info = "<html>
<p>
该模块将输出<strong>y</strong>，计算为输入<strong>u</strong>的<strong>cot</strong>：
</p>
<blockquote><pre>
y = <strong>cot</strong>( u );
</pre></blockquote>
<p>
如果u是pi的倍数，y = cot(u)为正负无穷大。
</p>
</html>"  ));
  end Cot;
...
如果输入信号为零或在计算的第一周期内，布尔输出信号<code>valid</code>变为<code>false</code>， 表示计算结果无效。有效的计算用<code>valid = true 。</code>
  ...
  model FFT "快速傅里叶变换"
    ...
    parameter Integer n = 1 "输入信号的长度";
    Modelica.Blocks.Interfaces.RealInput x[n] "输入信号" 
      annotation(Placement(transformation(origin = {-120, 0}, 
      extent = {{-20, -20}, {20, 20}})));
    Modelica.Blocks.Interfaces.RealOutput yr[n] "输出信号的实部" 
      annotation(Placement(transformation(origin = {110, 50}, 
      extent = {{-10, -10}, {10, 10}})));
    Modelica.Blocks.Interfaces.RealOutput yi[n] "输出信号的虚部" 
      annotation(Placement(transformation(origin = {110, -50}, 
      extent = {{-10, -10}, {10, 10}})));
    function FFT "快速傅里叶变换"
      input Real x[n];
      input Integer n;
      output Real yr[n];
      output Real yi[n];
  ...
  equation
    ...
    annotation(defaultComponentName = "FFT", 
      Documentation(info = "<html>
<p>
这个模型定义了一个快速傅里叶变换(Fast Fourier Transform, FFT)模型，接受长度为n的输入信号x[n]，
并生成转换后的实部和虚部作为输出。
</p>
</html>"    ), 
      Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}, 
  ...
  block RealFFT "对输入u进行采样和FFT"
    ...
    final parameter Integer ns = Modelica.Math.FastFourierTransform.realFFTsamplePoints(f_max, f_res, f_max_factor = 5) "采样点数";
  ...
  protected
    ...
    Real abs[nf](start = zeros(nf), each fixed = true) "FFT幅值";
...
并(在仿真结束时)通过函数<a href=\"modelica://Modelica.Math.FastFourierTransform.realFFTwriteToFile\">Math.realFFTwriteToFile</a>将输出写入结果文件resultFileName。
</p>
<p>
采样点数和采样周期根据所需的最大频率f_max和频率分辨率f_res计算得出。
</p>
<h4>注意</h4>
<p>
...
<p>
结果文件以mat格式写入，第一列为频率，第二列为幅值，第三列为相位。
频率点由幅度和相位均为0的行分隔，因此可以直接将结果绘制为频率线。
</p>
  ...
  block RealFFTWithOutput "对输入u进行采样和FFT，输出幅值和相位"
    extends Modelica.Blocks.Interfaces.DiscreteBlock(final samplePeriod = 1 / (2 * f_res * div(ns, 2)));
    parameter Units.SI.Frequency f_max "关注的最大频率";
    parameter Units.SI.Frequency f_res "频率分辨率";
    final parameter Integer ns = Modelica.Math.FastFourierTransform.realFFTsamplePoints(f_max, f_res, f_max_factor = 5) "采样点数";
    final parameter Integer nf = max(1, min(integer(ceil(f_max / f_res)) + 1, div(ns, 2))) "频率点数";
    parameter String resultFileName = "realFFT.mat" "结果文件：f，y_amplitudes，y_phases";
    output Integer info(start = 0, fixed = true) "FFT计算的状态标志";
    Modelica.Blocks.Interfaces.RealInput u 
      annotation(Placement(transformation(origin = {-120, 0}, 
      extent = {{-20, -20}, {20, 20}}), 
      ...
      extent = {{-20, -20}, {20, 20}})));
  protected
    Real buf[ns](start = zeros(ns), each fixed = true) "输入缓冲区";
    Integer iTick(start = 0, fixed = true) "采样时钟";
  public
    Interfaces.RealOutput y_amplitudes[nf](start = zeros(nf), each fixed = true) "FFT幅值" 
      annotation(Placement(transformation(origin = {110, 60}, 
      extent = {{-10, -10}, {10, 10}})));
    Interfaces.RealOutput y_phases[nf](start = zeros(nf), each fixed = true) "FFT相位" 
      annotation(Placement(transformation(origin = {110, -50}, 
      extent = {{-10, -10}, {10, 10}})));
  algorithm
    when {sampleTrigger} then
    ...
    when terminal() then
      if iTick < ns then
        assert(false, "采样时间不足! stopTime>= " + String(startTime + (ns - 1) * samplePeriod));
      else
        (info,y_amplitudes,y_phases) := Modelica.Math.FastFourierTransform.realFFT(buf, nf);
        assert(info == 0, "FFT计算出错");
