...
package Sources
  ...
  block RealExpression "将输出信号设置为随时间变化的实数表达式"
...
该模块的(随时间变化)实数输出信号可在其参数菜单中通过变量<strong> y </strong>进行定义。
其目的是支持在框图中轻松定义实数表达式。
例如，在y菜单中，可以给出\"<span style=\"color: rgb(51, 51, 51);\">if time &lt; 1 then 0 else 1</span>\"的定义，以确定如果time ≥ 1，输出信号为1，否则为0。
请注意，\"time\"是一个内置变量，可以随时访问，代表\"模型时间\"，而变量<strong> y </strong>既是一个变量，也是一个连接器。
</p>
</html>"      ));
  ...
  block IntegerExpression
    ...
    annotation(Icon(coordinateSystem(
      ...
      textString = "%name",
      textColor = {0, 0, 255})}), Documentation(info="<html><p>
该模块的整数输出信号(随时间变化)可通过变量<strong> y </strong>在参数菜单中定义。
其目的是支持在框图中简单定义整数表达式。
例如，在y菜单中，可以给出\"<span style=\"color: rgb(51, 51, 51);\">if time &lt; 1 then 0 else 1</span>\"的定义，以确定如果time ≥ 1，输出信号为 1，否则为 0。
请注意，\"time\"是一个内置变量，可以随时访问，代表\"模型时间\"，而变量<strong> y </strong>既是一个变量，也是一个连接器。
</p>
</html>"  ));
...
该模块布尔输出信号(随时间变化)可通过变量<strong> y </strong>在参数菜单中定义。
其目的是支持在框图中简单定义布尔表达式。
例如，在y菜单中，可以给出\"<span style=\"color: rgb(51, 51, 51);\">time &gt;= 1 and time &lt;= 2</span>\"的定义， 以确定输出信号在时间间隔 1 ≤ 时间 ≤ 2 时为<strong>true</strong>，否则为<strong>false</strong>。
请注意，\"time\"是一个内置变量，可以随时访问，代表\"模型时间\"，而变量<strong> y </strong>既是一个变量，也是一个连接器。
</p>
</html>"  ));
...
此元素用于生成控制器的参考信号，例如传动系统或者给定加速度的驱动接口。
  ...
  equation
    ...
    annotation(
      ...
      Documentation(info = "<html><p>
该模块通过表格<strong>线性插值</strong>生成输出信号。
时间点和函数值存储在矩阵<strong>table[i, j]</strong>中，其中第一列表[:, 1]包含时间点，第二列包含待插值的数据。
表格插值具有以下特性：
</p>
<ul><li>
...
"  , level = AssertionLevel.warning);
    ...
    annotation(
      Documentation(info = "<html>
<p>
这个模块通过<strong>常数插值</strong>、<strong>线性插值</strong>或<strong>三次Hermite样条插值</strong>在表格中生成输出信号y[:]。
时间点和函数值存储在矩阵<strong>table[i,j]</strong>中，其中第一列table[:,1]包含时间点，其他列包含待插值的数据。
</p>
...
<p>
通过参数<strong>columns</strong>可以定义表格中哪些列进行插值。
例如，如果columns={2, 4}，则假设有2个输出信号，第一个输出通过对表格矩阵第2列进行插值计算得到，第二个输出通过对表格矩阵第4列进行插值计算得到。
表格插值具有以下特性：
</p>
<ul>
<li>插值区间通过二分查找找到，其中上次调用使用的区间作为起始区间。</li>
<li>对于三次Hermite样条插值，时间点需要<strong>严格递增</strong>，否则需要<strong>单调递增</strong>。</li>
<li>对于常数插值或线性插值，通过在表格中提供相同的时间点两次，可以允许<strong>不连续性</strong>。</li>
...
<li>如果表格只有<strong>一行</strong>，则不执行插值，直接返回该行的表格值。</li>
<li>通过参数<strong>shiftTime</strong>和<strong>offset</strong>，可以将表格定义的曲线在时间和纵坐标上进行平移。
    因此，表格中存储的时间点是<strong>相对于</strong><strong>shiftTime</strong>的。</li>
<li>如果time &lt; startTime，则不进行插值，直接使用offset作为所有输出的纵坐标值。</li>
<li>该表格通过在区间边界生成<strong>时间事件</strong>，以数值稳定的方式实现线性段插值，从而为积分器生成连续可微的值。
通过参数<strong>timeEvents</strong>定义了时间事件的生成方式：
<blockquote><pre>
timeEvents = 1: Always generate time events at interval boundaries
    ...
    对于常数段插值，时间事件总是在区间边界生成。
    而对于平滑的三次Hermite样条插值，则不在区间边界生成时间事件。</li>
<li>通过参数<strong>timeScale</strong>可以对表格数组的第一列进行缩放，例如，如果表格数组是以小时为单位给出的(而不是秒)，
    则应将<strong>timeScale</strong>设置为3600。</li>
<li>对于特殊应用，有时需要知道表格中定义的最小和最大时间点作为参数。
    为此，提供了参数<strong>t_min</strong>/<strong>t_minScaled</strong>和<strong>t_max</strong>/<strong>t_maxScaled</strong>，
    可以从表格对象外部访问。
    其中，<strong>t_min</strong>和<strong>t_max</strong>定义了以SI.Time为单位、使用<strong>timeScale</strong>缩放后的横坐标值，
    而<strong>t_minScaled</strong>和<strong>t_maxScaled</strong>定义了表格的原始无量纲横坐标值。</li>
</ul>
<p>
示例：
</p>
...
<li>静态存储在文件\"usertab.c\"中的函数\"usertab\"中。
    矩阵通过\"tableName\"进行标识。
    参数fileName = \"NoName\"或仅包含空格。始终优先使用按行存储，因为否则矩阵将被重新分配并转置。</li>
</ol>
<p>
