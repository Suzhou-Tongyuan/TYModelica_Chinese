...
partial model GenericHysteresisTellinen "部分Tellinen滞后模型"
  extends BaseClasses.GenericHysteresis;

  //Real MagRel(start=0, min=-100, max=100) "初始化时的相对磁化率(-100-100%)";
  output Boolean asc(start=true) "当der(Hstat) > 0时为真";
protected
...
<p>采用Tellinen磁滞模型的磁阻。主要磁滞环由双曲正切函数定义。</p>
<h4>Tellinen滞后模型</h4>
<p>Tellinen磁滞模型是描述铁磁材料磁滞行为的一个简单模型。它只使用主要磁滞回线的上升（hystR）和下降（hystF）分支及其导数 der(hystR) 和 der(hystF)。有关Tellinen磁滞模型的简短描述，请参见图1和以下公式。</p>
<blockquote><pre>
diffHyst = hystF - hystR;
dhR = hystF - b;
dhF = b - hystR;
...
<strong>Fig. 1:</strong> Tellinen滞后模型说明。
</html>"));
