...
<p>
与电动力或<em>洛伦兹</em>力F，变换器常数c，电流i，感应反电动势V_i和电枢速度v，该模型非常类似于众所周知的旋转单相直流电机的行为模型，除了它是平移运动。
对于在气隙中有线圈、磁通密度为B、磁场内总导线长度为l的动圈执行器，变换器常数为
</p>
...
<p>
假设变换器常数c以及线圈电阻R和电感L是已知的，例如，从测量或目录数据。
因此，该模型非常适合与邻近子系统一起进行系统仿真，但不适用于执行器设计，其中电机常数是根据磁路的几何形状，材料特性和绕组数据找到的。
参见<a href=\"modelica://Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.Components. PermeanceActuator\">PermeanceActuator</a>是基于磁网络的更精确的执行器模型。
由于相同的连接器，这两个模型都可以用于系统仿真，例如模拟一个行程，
如<a href=\"modelica://Modelica.Magnetic.FluxTubes.Examples.MovingCoilActuator.ArmatureStroke\">ArmatureStroke</a>所示。
</p>
</html>"));
