...
model Transformer1PhaseWithHysteresis
  ...
  parameter SI.Length l1(displayUnit="mm") = 40e-3
    "核心平均长度 l1" annotation (Dialog(tab="铁芯", group="几何结构", groupImage="modelica://Modelica/Resources/Images/Magnetic/FluxTubes/Examples/Hysteresis/Components/Transformer1PhaseWithHysteresis/Core_SinglePhase.png"));
  ...
  parameter FluxTubes.Material.HysteresisEverettParameter.BaseData mat=
    ...
    "铁磁磁滞参数集" annotation (Dialog(tab="铁芯",
        group="材料"), choicesAllMatching=true);
  ...
  parameter Real MagRelStart=0 "磁芯初始磁化 (-1..1)" 
    annotation (Dialog(tab="铁芯", group="初始化"));
  parameter Boolean MagRelFixed = false "固定" annotation (Dialog(tab="铁芯", group="初始化"),choices(checkBox=true));

  parameter SI.MagneticFieldStrength HStart=0
    "磁芯的初始磁场强度" 
    annotation (Dialog(tab="铁芯", group="初始化"));
  parameter Boolean HFixed = false "固定" 
    annotation (Dialog(tab="铁芯", group="初始化"),choices(checkBox=true));
  parameter SI.ElectricCurrent I1Start=0
    "通过绕组的初始初级电流" annotation (Dialog(tab="铁芯", group="初始化"));
  parameter Boolean I1Fixed = false "固定" annotation (Dialog(tab="铁芯", group="初始化"),choices(checkBox=true));

  output SI.Power LossPowerWinding "绕组损耗";
  output SI.Power LossPowerStat "铁磁磁滞损耗";
  output SI.Power LossPowerEddy "涡流损耗";

  extends Interfaces.ConditionalHeatPort(useHeatPort=false);
  parameter Boolean EddyCurrents = false "启用涡流" 
    annotation(Dialog(tab="损耗与发热", group="涡流"), choices(checkBox=true));
  ...
  parameter SI.Length t(displayUnit="mm") = 0.5e-3
    "层压厚度" annotation (Dialog(tab="损耗与发热", group="涡流", enable=EddyCurrents));
  ...
  parameter SI.Length L_l2=10e-3
    "次级绕组泄漏长度" annotation (Dialog(tab="漏磁"));
