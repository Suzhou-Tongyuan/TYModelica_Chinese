...
model SMEE_Generator 
  ...
  parameter SI.Angle gamma0(displayUnit="deg") = 0 
    "转子初始位移角";
  SI.Angle thetaQS=rotorAngleQS.rotorDisplacementAngle "转子位移角，准静态";
  SI.Angle theta=rotorAngle.rotorDisplacementAngle "转子位移角，瞬态";

  output SI.Power Ptr=powerSensor.power "瞬态动力";
  output SI.Power Pqs=powerSensorQS.apparentPower.re "QS动力";
...
<p>
本例比较了电激同步电机的时间暂态模型和准静态模型。
电励磁同步发电机连接到电网并以恒速驱动。
由于速度略小于与主频率对应的同步速度，
转子角度的增加非常缓慢。这允许看到依赖于转子角度的几个特性.
</p>

<p>
模拟30秒，情节对比 <code>rotorAngle|rotorAngleQS.rotorDisplacementAngle</code>:
</p>

<ul>
<li><code>smpm|smpmQS.tauElectrical</code>: 机转矩</li>
</ul>

<p>由于转子转差很低，因此瞬态和准静态电磁转矩几乎相等.</p>
</html>"), 
