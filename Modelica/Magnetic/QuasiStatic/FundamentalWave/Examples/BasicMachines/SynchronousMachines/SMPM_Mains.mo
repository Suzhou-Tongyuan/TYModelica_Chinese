...
model SMPM_Mains 
  ...
  parameter SI.Voltage V=112.3 "电源电压";
  parameter SI.Torque T_Load=181.4 "额定负载扭矩";
  parameter SI.Time tStep=0.5 "负载转矩阶跃时间";
  ...
  parameter
    ...
    smpmData(useDamperCage=true) "同步机数据" 
...
<p>
本例比较了永磁同步电机的时间暂态模型和准静态模型。机器以零负载和同步速度启动。在时间<code>tStep</code>，机器加载公称扭矩.</p>
