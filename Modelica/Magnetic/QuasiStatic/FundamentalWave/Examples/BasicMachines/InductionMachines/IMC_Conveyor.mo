...
model IMC_Conveyor "感应电机与鼠笼和变频器驱动的输送机"
...
equation
  ...
  annotation (experiment(StopTime=20, Interval=0.0001, Tolerance=1e-06), Documentation(
        info="<html>
<p>
利用VfController和三相信号电压对理想的变频器进行了建模。
频率由加速、匀速、减速和静止的负载循环驱动。
机械负载是一个恒定的扭矩，就像一个输送机(在零速度附近正则化).
</p>
<p>模拟20秒并绘制(相对于时间):</p>
<ul>
<li>currentQuasiRMSSensor|currentQuasiRMSSensorQS.I: 定子电流均方根</li>
<li>imc|imcQS.wMechanical: 机速度</li>
