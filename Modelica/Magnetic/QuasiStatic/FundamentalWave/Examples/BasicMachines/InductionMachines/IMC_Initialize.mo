...
model IMC_Initialize "鼠笼式感应电机的稳态初始化"
  extends Modelica.Icons.Example;
  import Modelica.Constants.pi;
  parameter Integer m=3 "相数" annotation(Evaluate=true);
  parameter SI.Voltage VNominal=100 
    "每相标称均方根电压";
  parameter SI.Frequency fNominal=imc.fsNominal "标称频率";
  parameter SI.AngularVelocity wSync=2*pi*fNominal/imcData.p "同步转速";
...
equation
  ...
  annotation (experiment(StopTime=1.5, Interval=0.00001, Tolerance=1e-06), Documentation(
        info="<html>
<strong>测试示例:带鼠笼感应电机的稳态初始化</strong><br>
鼠笼式感应电机在空载状态下初始化为稳态;
在tStart时刻，施加负载转矩步骤.<br>
模拟1.5秒并绘制(相对于时间):
<ul>
...
使用型号<em>IM_SquirrelCage</em>的默认机器参数.
</html>"), 
    Diagram(graphics={
        Text(
          extent={{20,8},{100,0}}, 
