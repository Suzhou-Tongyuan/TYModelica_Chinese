...
model RotorDisplacementAngle "转子滞后角"
  ...
  parameter Boolean useSupport=false "使用支撑或固定外壳" 
  ...
  Modelica.Mechanics.Rotational.Interfaces.Flange_a support if useSupport 
    "反作用扭矩作用的支撑点" annotation (Placement(
...
<p>
通过测量定子相电压，将其转换为定子固定坐标系中相应的空间相位来计算转子滞后角、
将空间相位旋转到转子固定坐标系，并计算该空间相位的角度.</p>
<p>
传感器的外壳可以隐式固定(<code>useSupport=false</code>)。
如果机器的定子也隐式固定(<code>useSupport=false</code>)，则在法兰处的角度
等于电机转子与定子的夹角。
否则，传感器的支架必须连接到机器的支架上.
</p>
</html>"));
