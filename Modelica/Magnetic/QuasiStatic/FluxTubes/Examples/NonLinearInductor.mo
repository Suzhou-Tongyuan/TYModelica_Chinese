...
model NonLinearInductor "带铁磁磁芯的非线性电感器"
  ...
  Modelica.Electrical.QuasiStatic.SinglePhase.Basic.Resistor 
    rQS(R_ref=7.5) "电感器线圈电阻" 
                               annotation (Placement(transformation(extent={{-50,-50}, 
            {-30,-30}})));
  FluxTubes.Basic.ElectroMagneticConverter coilQS(N=600) "电感器线圈" annotation (Placement(transformation(extent={{-20,-60},{0,-40}})));
...
connect(rootMeanSquare.y_rms, feedback.u1) annotation (Line(
  ...
  annotation (experiment(
      ...
      Tolerance=1e-07),                                   Documentation(
        info="<html>
<p>
该模型比较了瞬态非线性磁路和线性化准静态磁路。正弦电压被施加到带有矩形封闭铁磁磁芯的电感器上。.
</p>
