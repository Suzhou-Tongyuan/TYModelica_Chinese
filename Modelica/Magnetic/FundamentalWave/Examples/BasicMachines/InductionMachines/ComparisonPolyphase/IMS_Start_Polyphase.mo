...
model IMS_Start_Polyphase 
  ...
  parameter Integer mr=5 "转子相数" annotation(Evaluate=true);
...
两台机器从静止状态启动，加速惯量对负载转矩的二次依赖于速度，
使用启动电阻。此时变阻器外转子电阻缩短，最终达到标称转速。比较了两种不同相数的等效电机，证明了它们的等效性能.</p>
