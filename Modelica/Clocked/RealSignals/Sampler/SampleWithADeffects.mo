...
block SampleWithADeffects 
  ...
  parameter Boolean noisy = false 
    "= true，如果输出应与噪声叠加" 
    annotation(Evaluate = true, choices(checkBox = true), Dialog(group = "采样和噪声"));
  ...
  parameter Integer bits(min = 1) = 8 
    "量化位数（如果quantized = true）" annotation(Dialog(enable = limited and quantized, group = "限制和量化"));
  ...
  replaceable Clocked.RealSignals.Sampler.Utilities.Internal.UniformNoise noise if noisy 
    ...
    Dialog(enable = noisy, group = "采样和噪声"), 
