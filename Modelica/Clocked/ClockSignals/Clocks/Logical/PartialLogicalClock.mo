...
partial block PartialLogicalClock 
  "按照可替换的逻辑组合器将任意多个输入时钟信号组合在一起的逻辑时钟"
  extends Interfaces.PartialClock;

  parameter Integer nu = 2 "输入连接数";

  Interfaces.ClockVectorInput u[nu] "时钟输入信号的向量" 
  ...
  replaceable Modelica.Blocks.Interfaces.PartialBooleanMISO combinator 
    ...
    nu = nu) 
    "应用于时钟输入信号向量的可替换逻辑组合器。
    重要：不得使用任何形式的否定运算，因为这会导致在极短的时间段内产生无限多个时钟脉冲。" 
