...
package IdealGases "基于NASA热力学数据库的理想气体模型及数据集(涵盖单一组分、固定配比与动态混合体系)"
  extends Modelica.Icons.VariantsPackage;
  annotation (Documentation(info="<html><p>
该库包含1241种理想气体的数据
</p>
<p>
McBride B.J., Zehe M.J., and Gordon S. (2002): <strong>NASA Glenn Coefficients for Calculating Thermodynamic Properties of Individual Species</strong>. NASA report TP-2002-211556
</p>
<p>
一些气体的介质模型可以在<a href=\"modelica://Modelica.Media.IdealGases.SingleGases\" target=\"\">IdealGases.SingleGases</a>&nbsp;库中找到，一些混合物的示例可以在<a href=\"modelica://Modelica.Media.IdealGases.MixtureGases\" target=\"\">IdealGases.MixtureGases</a>&nbsp;库中找到
</p>
<h4>使用和调整介质模型</h4><p>
数据记录允许计算下列物质的理想气体比焓、比熵和比热容。甚至可以从中计算反应的吉布斯能量和平衡常数。用于计算黏度和导热系数的关键数据未包含在内。为了添加介质库（混合物或单一物质介质库）作为<a href=\"modelica://Modelica.Media.Interfaces.PartialMedium\" target=\"\">Interfaces.PartialMedium</a>&nbsp;的子类型（即可以在所有定义了PartialMedium的地方使用），需要执行一些额外的步骤：
</p>
<ol><li>
所有单一气体介质都需要定义一个常量实例记录<a href=\"modelica://Modelica.Media.Interfaces.PartialMedium.FluidConstants\" target=\"\">IdealGases.Common.SingleGasNasa.FluidConstants</a>&nbsp;。对于 37 种理想气体，<a href=\"modelica://Modelica.Media.IdealGases.Common.FluidData\" target=\"\">IdealGases.Common.FluidData</a>&nbsp;中提供了此类记录。对于其他气体，用户必须提供这样一个记录实例，例如通过从商业或公共数据库获取数据。所需数据的公共来源库括<a href=\"http://webbook.nist.gov/chemistry/\" target=\"\"> NIST Chemistry WebBook</a>&nbsp;</li>
<li>
当数据可用且用户有一个填充有数据的<a href=\"modelica://Modelica.Media.Interfaces.PartialMedium.FluidConstants\" target=\"\">FluidConstants</a>&nbsp;记录的实例时，必须编写介质库。请注意，黏度和导热系数函数仅需要偶极矩、偏心因子和临界数据。</li>
<li>
对于单组分，必须创建一个新库，遵循IdealGases.SingleGases中的模式，同时指向一个cp数据记录和用户定义的fluidConstants记录。对于多组分混合物，必须创建一个新库，遵循IdealGases.MixtureGases中的模式，构建一个cp数据记录数组和一个（部分）用户定义的fluidConstants记录数组。</li>
</ol><p>
请注意，可以计算以下1241种气体的完整性质，但由于缺少黏度和导热系数函数，无法定义完全符合Modelica.Media的介质。
</p>
<p>
以下物质和离子的热容、比焓和比熵的数据记录存在：
</p>
<pre><code >Ag        BaOH+           C2H4O_ethylen_o DF      In2I4    Nb      ScO2
