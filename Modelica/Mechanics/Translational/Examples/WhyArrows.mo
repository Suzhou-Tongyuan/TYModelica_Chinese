...
model WhyArrows "在 Mechanics.Translational 中使用箭头"
...
<p>
在使用 Mechanics.Translational 子库的模型时，
建议确保所有箭头指向同一方向，因为这样所有组件都将具有相同的参考系。
在本示例中，Rod1的flange_a到Rod2的flange_b的距离为2米。
Rod1的flange_a到Rod3的flange_b的距离也是2米，尽管这点很难看出来。
如果没有箭头，几乎不可能注意到这一点。
确保所有箭头指向同一方向是使用该库的充分条件。
有些情况下可以使用水平翻转的模型而不会出现问题。
</p>
</html>"), 
