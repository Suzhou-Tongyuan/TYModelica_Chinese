...
model InitialConditions "设置初始条件"
...
<p>
有几种设置初始条件的方法。
在第一个系统中，质量m3的位置通过使用修改器s(start=4.5)定义，
质量m4的位置通过s(start=12.5)定义。选择这些位置使系统静止。
要计算这些值，请从左边（fixed2）开始，值为1m。
弹簧s2的未拉伸长度为2m，m3的长度为3m，导致
</p>

<blockquote><pre>
  1   m（fixed2）
+ 2   m（spring s2）
+ 3/2 m（half of length）
-------
...
<p>
选择这些初始条件可以优先选择那些变量（m3.s 和 m4.s）作为状态变量。
</p>

<p>
在第二个例子中，弹簧的长度给出了起始值，但它们不能用作纯弹簧的状态（仅用于弹簧/阻尼器组合）。
在这种情况下，系统不处于静止状态。
</p>
