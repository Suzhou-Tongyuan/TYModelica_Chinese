...
partial model PartialTwoFlangesAndSupport 
  "具有两个一维平动接口和一个支撑组件的部分模型，用于图形建模，即模型是通过从基本组件中拖放构建"
...
这是一个具有两个一维平动接口和一个支撑/壳体的一维传动组件的部分模型。 它用于以图形方式构建由多个组件组成的传动系统的部件。
</p>
<p>
如果 <em>useSupport=true</em>，则支撑连接器被有条件地启用并且需要连接。
</p>
<p>
