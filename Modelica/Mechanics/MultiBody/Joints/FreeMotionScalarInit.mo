...
model FreeMotionScalarInit 
  ...
  parameter Boolean use_r = false "=true，如果要使用r_rel_a" 
    annotation(Evaluate = true, HideResult = true, Dialog(tab = "平动初始化", group = "r_rel_a是从frame_a的原点到frame_b的原点的位置矢量，在frame_a中解析"));
  Modelica.Blocks.Interfaces.RealOutput r_rel_a_1(final quantity = "Length", final unit = "m", start = 0, final stateSelect = r_rel_a_1_stateSelect) if use_r 
    "相对距离r_rel_a[1]" 
    annotation(Dialog(enable = use_r, tab = "平动初始化", group = "r_rel_a是从frame_a的原点到frame_b的原点的位置矢量，在frame_a中解析", showStartAttribute = true));
  ...
  parameter StateSelect r_rel_a_1_stateSelect = StateSelect.never 
    "r_rel_a[1]的状态选择" annotation(HideResult = true, 
    Dialog(enable = use_r, tab = "平动初始化", group = "r_rel_a是从frame_a的原点到frame_b的原点的位置矢量，在frame_a中解析"));
  parameter StateSelect r_rel_a_2_stateSelect = StateSelect.never 
    "r_rel_a[2]的状态选择" annotation(HideResult = true, 
    Dialog(enable = use_r, tab = "平动初始化", group = "r_rel_a是从frame_a的原点到frame_b的原点的位置矢量，在frame_a中解析"));
  parameter StateSelect r_rel_a_3_stateSelect = StateSelect.never 
    "r_rel_a[3]的状态选择" annotation(HideResult = true, 
    Dialog(enable = use_r, tab = "平动初始化", group = "r_rel_a是从frame_a的原点到frame_b的原点的位置矢量，在frame_a中解析"));

  parameter Boolean use_v = false "=true，如果要使用v_rel_a" 
    annotation(Evaluate = true, HideResult = true, 
    Dialog(enable = use_r, tab = "平动初始化", group = "速度矢量v_rel_a=der(r_rel_a)"));
  Modelica.Blocks.Interfaces.RealOutput v_rel_a_1(final quantity = "Velocity", final unit = "m/s", start = 0, final stateSelect = v_rel_a_1_stateSelect) if use_r and use_v 
    "相对速度v_rel_a[1]" 
    annotation(Dialog(enable = use_r and use_v, tab = "平动初始化", group = "速度矢量v_rel_a=der(r_rel_a)", showStartAttribute = true));
