...
model Body "具有质量、转动惯量张量和一个坐标系连接器的刚体(12个潜在状态变量)"
  ...
  parameter SI.Angle angles_start[3] = {0, 0, 0} 
    "将世界坐标系绕'sequence_start'轴旋转到frame_a的初始角度值" 
    annotation(Dialog(tab = "初始值"));
  parameter Types.RotationSequence sequence_start = {1, 2, 3} 
    "将世界坐标系旋转到frame_a的初始时间的旋转序列" 
  ...
  parameter Types.RotationSequence sequence_angleStates = {1, 2, 3} 
    "将世界坐标系绕使用作为潜在状态变量的3个角度的轴旋转到frame_a的旋转序列" 
  ...
  // 世界坐标系中的重力加速度
  SI.Acceleration g_0[3] "在世界坐标系中解析的重力加速度";
...
protected
  ...
  Frames.Quaternions.Orientation Q(start = Q_start, each stateSelect = if 
    ...
    "世界坐标系到frame_a的四元数定向对象(如果不使用四元数作为状态，则为虚拟值)";
...
物体的潜在状态变量包括：</p>
<ul>
<li>从世界坐标系的原点到物体frame_a的原点的<strong>位置矢量</strong>frame_a.r_0，解析为世界坐标系和frame_a原点的<strong>绝对速度</strong>v_0，解析为世界坐标系(=der(frame_a.r_0))。
</li>
...
frame_a的方向是通过沿参数矢量\"sequence_angleStates\"(默认值={1,2,3}，即Cardan角序列)定义的轴旋转世界坐标系来计算的，围绕作为潜在状态变量的角度旋转。
例如，默认情况下，围绕angles[1]旋转世界坐标系的x轴，围绕angles[2]旋转新的y轴，围绕angles[3]旋转新的z轴，得到frame_a。
</li>
</ul>
