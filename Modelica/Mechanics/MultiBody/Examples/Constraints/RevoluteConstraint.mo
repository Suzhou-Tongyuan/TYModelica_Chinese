...
<p>
本示例展示了代表<strong>转动副</strong>的<strong>约束</strong>功能。
两个刚体中的每一个都通过弹簧连接到世界的一个端点。
另一个端点也通过转动副或适当的约束连接到世界。
因此，刚体只能根据作用力围绕旋转轴进行旋转运动。
</p>

<p>
<strong>模拟结果</strong></p>
<p>
在模拟模型后，观察多体系统的动画，并比较通过运动副连接的刚体(蓝色)与通过约束连接的刚体(绿色)的运动。
