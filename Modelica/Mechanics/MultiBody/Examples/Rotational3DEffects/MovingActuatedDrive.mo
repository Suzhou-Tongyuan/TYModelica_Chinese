...
<p>
这个模型展示了如何使用
<a href=\"modelica://Modelica.Mechanics.MultiBody.Parts.Rotor1D\">rotor1D</a>组件来替代使用三维多体元素(如旋转副和圆柱体)
建模的移动传动系统，以加快模拟速度。两种系统的运动是完全相同的，
并且在各个坐标系中的局部扭矩(例如：r1.frame_b.t和r2.frame_b.t)也相同。
</p>

<p>
驱动关节(r1、r2)的旋转轴为{0,1,0}，这些关节被建模为由沿{1,0,0}轴作用的电机扭矩驱动。这基本意味着使用了理想化的锥齿轮来驱动旋转关节的轴。
</p>
</html>"));
