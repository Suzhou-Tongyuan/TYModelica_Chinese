...
model Torus "可视化一个环面"
  ...
  parameter SI.Radius R=0.5 "主半径(从环面中心到管道中心的距离)" annotation(Dialog(enable=animation));
  parameter SI.Radius r=0.1 "次半径(管道的半径)" 
    annotation(Dialog(enable=animation));
  parameter SI.Angle opening=0 "环面的开口角度" annotation(Dialog(enable=animation));
  parameter SI.Angle startAngle=-3.1415926535898 
    "环面切片的起始角度" annotation(Dialog(enable=animation));
  parameter SI.Angle stopAngle=3.1415926535898 
    "环面切片的结束角度" annotation(Dialog(enable=animation));
  ...
  input Modelica.Mechanics.MultiBody.Types.RealColor color={0,128,255} 
    "表面颜色" annotation(Dialog(enable=animation, colorSelector=true, group="材料属性"));
  input Types.SpecularCoefficient specularCoefficient = 0.7 
    "环境光反射(=0:光线完全被吸收)" annotation(Dialog(enable=animation,group="材料属性"));
  input Real transparency=0 
    "形状的透明度：0(=不透明)...1(=完全透明)" 
                               annotation(Dialog(enable=animation,group="材料属性"));
  parameter Integer n_R=40 "主半径R上的点数" annotation(Dialog(enable=animation,tab="离散化"));
  parameter Integer n_r=20 "次半径r上的点数" annotation(Dialog(enable=animation,tab="离散化"));
...
<strong>环面</strong>模型用于可视化一个环面。
环面的中心位于连接器frame_a处(在下图中由红色坐标系表示)。
下面左图显示了主半径<var>R</var>=0.5m和次半径<var>r</var>=0.2m的环面。
下面右图显示了具有以下附加参数设置的环面：</p>
<blockquote><pre>
开口角度=45度
起始角度=-135度
结束角度=135度
...
<p>
在高级菜单中，可以通过主半径上的点数<var>n<sub>R</sub></var>和次半径上的点数<var>n<sub>r</sub></var>来定义曲面可视化的离散化。
如果环面是闭合的(即，<code>开口角度</code>=0度)，则实际点数会减少一个(即<var>n<sub>R</sub></var>-1,<var>n<sub>r</sub></var>-1)，因为在这种情况下，参数化的第一个点和最后一个点重合。
</p>
</html>", 
        revisions="<html>
<ul>
<li>2010年7月，由Martin Otter进行修改<br>
适应了新的表面模型。
</li>
<li>2005年7月，由Dirk Zimmer(在德国航空航天中心进行实习)进行编写<br>
第一个可视化多级轮胎滚轮模型的版本。
</li>
</ul>
