...
model PipeWithScalarField 
  ...
  parameter SI.Radius rOuter "管道的外半径" annotation(Dialog(enable=animation));
  parameter SI.Length length "管道的长度" annotation(Dialog(enable=animation));

  parameter Real xsi[:](each min=0, each max=1)= Modelica.Math.Vectors.relNodePositions(12) 
    "沿着管道的相对位置的矢量，其中x[1]=0，x[end]=1" 
    annotation(Dialog(enable=animation));
  input Real T[size(xsi,1)] 
    "在位置xsi*length处的值的矢量(将通过颜色进行可视化)" annotation(Dialog(enable=animation));
  parameter Real T_min "对应于colorMap[1,:]的T的最小值" annotation(Dialog(enable=animation));
  parameter Real T_max 
    "对应于colorMap[end,:]的T的最大值" annotation(Dialog(enable=animation));
  replaceable function colorMap = 
    ...
    "定义颜色映射的函数" 
    annotation(choicesAllMatching=true, Dialog(enable=animation,group="颜色编码"), 
    Documentation(info="<html>
<p>
此可替换函数定义了特定的颜色映射。
</p>
<!--aplaceholdertofulfillminimumdocumentationlength-->
</html>"));

  parameter Integer n_colors=64 "colorMap中的颜色数量" annotation(Dialog(enable=animation,group="颜色编码"));
  parameter Types.SpecularCoefficient specularCoefficient = 0.7 
    "环境光的反射(=0：光线完全被吸收)" annotation(Dialog(enable=animation,group="颜色编码"));
  parameter Real transparency=0 
    "形状的透明度：0(=不透明)...1(=完全透明)" 
    annotation(Dialog(enable=animation,group="颜色编码"));

  parameter Integer n_rOuter=30 "外半径上的点数量" annotation(Dialog(enable=animation,tab="离散化"));
  parameter Integer n_length=20 "长度上的点数量" annotation(Dialog(enable=animation,tab="离散化"));
...
equation
  ...
  Documentation(info = "<html>
<p>
<strong>PipeWithScalarField</strong>模型可视化了一个管道以及沿着管道轴线的标量场。
后者通过将标量场映射到颜色值并利用该颜色值在与相应轴位置相关联的周边显示。
通常标量场值是温度，但也可能是其他量。
预定义的颜色映射可从<a href=\"modelica://Modelica.Mechanics.MultiBody.Visualizers.Colors.ColorMaps\">MultiBody.Visualizers.Colors.ColorMaps</a>获取，并可以通过参数\"colorMap\"进行选择。
具有相应标量场值的颜色映射可以使用函数<a href=\"modelica://Modelica.Mechanics.MultiBody.Visualizers.Colors.colorMapToSvg\">MultiBody.Visualizers.Colors.colorMapToSvg</a>以svg格式导出为矢量图形。
该组件的连接器frame_a位于管道左侧圆圈的中心，并且管轴线沿着frame_a的x轴定向，参见下图，在该图中frame_a被可视化为一个坐标系：</p>
...
<p>
下图显示了颜色编码。
它是用<a href=\"modelica://Modelica.Mechanics.MultiBody.Visualizers.Colors.colorMapToSvg\">MultiBody.Visualizers.Colors.colorMapToSvg</a>
使用以下调用：</p>
...
<img src=\"modelica://Modelica/Resources/Images/Mechanics/MultiBody/Visualizers/PipeWithScalarField-ColorMap.png\">
        ...
        revisions="<html>
<ul>
<li>July2010byMartinOtter<br>
AdaptedtothenewSurfacemodel.</li>
<li>July2005byDirkZimmer(practicaltrainingatDLR)<br>
Firstversiontovisualizeamulti-leveltyrewheelmodel.</li>
</ul>
</html>"));
