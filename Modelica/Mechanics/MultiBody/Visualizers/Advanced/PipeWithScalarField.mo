...
model PipeWithScalarField "可视化带有标量场的管道"
  input Frames.Orientation R=Frames.nullRotation() 
    "用于将全局坐标系旋转到表面坐标系的方向对象" 
    annotation(Dialog(group="表面坐标系"));
  input SI.Position r_0[3]={0,0,0} 
    "从全局坐标系原点到表面坐标系原点的位置矢量，以全局坐标系解析" 
    annotation(Dialog(group="表面坐标系"));

  parameter SI.Radius rOuter "管道外半径" annotation(Dialog(group="表面属性"));
  parameter SI.Length length "管道长度" annotation(Dialog(group="表面属性"));
  parameter Integer n_rOuter=30 "沿外半径方向的点数" annotation(Dialog(group="表面属性"));
  parameter Integer n_length=10 "沿长度方向的点数" annotation(Dialog(group="表面属性"));
  parameter Types.SpecularCoefficient specularCoefficient = 0.7 
    "环境光反射率(=0：光完全被吸收)" annotation(Dialog(group="表面属性"));
  parameter Real transparency=0 
    "形状的透明度：0(不透明)...1(完全透明)" 
    annotation(Dialog(group="表面属性"));

  parameter Real xsi[:](each min=0,each max=1)= Modelica.Math.Vectors.relNodePositions(12) 
    "沿管道的相对位置矢量，其中x[1]=0，x[end]=1" 
    annotation(Dialog(group="颜色编码"));
  input Real T[size(xsi,1)] 
    "在xsi*length处的数值矢量(将通过颜色进行可视化)" annotation(Dialog(group="颜色编码"));
  parameter Real T_min 
    "对应于colorMap[1,:]的T的最小值" annotation(Dialog(group="颜色编码"));
  parameter Real T_max 
    "对应于colorMap[end,:]的T的最大值" annotation(Dialog(group="颜色编码"));
  parameter Integer n_colors=64 "colorMap中的颜色数量" annotation(Dialog(group="颜色编码"));
  replaceable function colorMap = 
    ...
    "定义颜色映射的函数" 
    annotation(choicesAllMatching=true, Dialog(group="颜色编码"), 
...
protected
  ...
  annotation(Icon(graphics={
   ...
   Documentation(info = "<html>
<p>
模型<strong>PipeWithScalarField</strong>可视化了管道和沿管道轴的标量场。
后者通过将标量场映射到颜色值并利用这种颜色在与相应轴位置相关联的周边显示。
通常，标量场值是温度，但也可能是其他数量。
预定义的颜色映射可从<a href=\"modelica://Modelica.Mechanics.MultiBody.Visualizers.Colors.ColorMaps\">MultiBody.Visualizers.Colors.ColorMaps</a>获取，并可以通过参数\"colorMap\"选择。
带有相应标量场值的颜色映射可以通过函数<a href=\"modelica://Modelica.Mechanics.MultiBody.Visualizers.Colors.colorMapToSvg\">MultiBody.Visualizers.Colors.colorMapToSvg</a>以svg格式导出为矢量图。
左端圆的位置和方向的定义通过参数\"r_0\"和\"R\"，分别指定。
管道轴沿着由\"R\"描述的局部坐标系的x轴方向，如下图所示：</p>
...
<p>
颜色编码显示在下图中。
它是使用以下调用生成的<a href=\"modelica://Modelica.Mechanics.MultiBody.Visualizers.Colors.colorMapToSvg\">MultiBody.Visualizers.Colors.colorMapToSvg</a>：</p>
