...
这是一个理想的齿轮箱其质量和惯性为零，将一维旋转运动转换为一维平移运动。如果需要考虑弹性、阻尼或游隙，则必须将此理想齿轮箱连接到相应的元件。 此外此组件定义了如下的运动学约束：
</p>
<pre><code >(flangeR.phi - internalSupportR.phi) = ratio*(flangeT.s - internalSupportT.s);
</code></pre><p>
<br>
</p>
</html>"), 
