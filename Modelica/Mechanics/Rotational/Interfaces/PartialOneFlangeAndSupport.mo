...
partial model PartialOneFlangeAndSupport 
    "具有一个一维转动接口和一个用于图形建模的支撑组件的部分模型，即通过从基本组件拖放构建模型"
  ...
  Flange_b flange "轴一维转动接口" annotation (Placement(transformation(
          extent={{90,-10},{110,10}})));
...
<p>
这是一个具有一个一维转动接口和一个支撑/轴承座的1维旋转组件。
它用于以图形方式构建由多个组件组成的传动系统的部件。
</p>

<p>
如果<em>useSupport=true</em>，则支撑连接器被有条件地启用且需要被连接。<br>
